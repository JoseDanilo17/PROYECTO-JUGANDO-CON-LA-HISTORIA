window.addEventListener("DOMContentLoaded", () => {
    ...
    // ======== PALABRAS POR NIVEL =====
    const levels = {
        facil: { words: ["MUISCAS","TAYRONAS","QUIMBAYA","ARHUACOS","GUANE","PASTOS","TUNJA","CACIQUE"], directions: ["H","V"] },
        medio: { words: ["SOGAMOSO","BOYACÁ","ORFEBRERIA","MOMIA","ZENU","CHIBCHAS"], directions: ["H","HR","V","VR"] },
        dificil: { words: ["BACHUE","GUATAVITA","MITO","DORADO"], directions: ["H","HR","V","VR","D","DR"] }
    };
    ...
    const dirMap = { H:[1,0], HR:[-1,0], V:[0,1], VR:[0,-1], D:[1,1], DR:[-1,-1] };

    function createEmptyGrid(){ board = Array(gridSize).fill(null).map(()=>Array(gridSize).fill("")); }
    function placeWord(word){
        ...
        while(!placed && attempts<200){
            ...
            for(let i=0;i<word.length;i++){
                ...
                if(board[ny][nx]!=="" && board[ny][nx]!==word[i]){ ok=false; break; }
            }
            if(!ok) continue;
            for(let i=0;i<word.length;i++){
                let nx=x+dx*i, ny=y+dy*i;
    ...
    function selectLetter(div){
        let x=div.dataset.x, y=div.dataset.y, id=`${x},${y}`;
        if(selected.includes(id)){
            selected=selected.filter(i=>i!==id); div.classList.remove("selected"); return;
        }
        selected.push(id); div.classList.add("selected"); checkWord();
    }

    function checkWord(){
        let formed=selected.map(i=>{ let [x,y]=i.split(",").map(Number); return board[y][x]; }).join("");
        for(let w of words){ if(formed.includes(w)){ markAsFound(w); return; } }
    }
    ...
    function markAsFound(word){
        ...
        if(li) li.style.color="#00cc44";

        // Guardar en localStorage
        let savedWords=JSON.parse(localStorage.getItem("foundWords")||"[]");
        if(!savedWords.includes(word)) savedWords.push(word);
